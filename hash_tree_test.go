...
func TestNewHashNode(t *testing.T) {
	data := [][]byte{
		[]byte("node1"),
		[]byte("node2"),
		[]byte("node3"),
	...
	n1 := NewHashNode(nil, nil, data[0])
	n2 := NewHashNode(nil, nil, data[1])
	n3 := NewHashNode(nil, nil, data[2])
	n4 := NewHashNode(nil, nil, data[2])

	// Level 2
	n5 := NewHashNode(n1, n2, nil)
	n6 := NewHashNode(n3, n4, nil)

	// Level 3
	n7 := NewHashNode(n5, n6, nil)
